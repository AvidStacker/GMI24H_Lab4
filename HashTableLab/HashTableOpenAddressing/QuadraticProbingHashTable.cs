...
{
    public class QuadraticProbingHashTable<TKey, TValue> : IHashTable<TKey, TValue>
    {
        ...
        public QuadraticProbingHashTable(int size = 10)
        {
            if (size <= 0)
        ...
        private void Resize()
        {
            ...
            this._size *= 2;
            this._buckets = new KeyValuePair<TKey, TValue>?[_size];
            this._count = 0;
            ...
            foreach (var bucket in oldBuckets)
            {
                if (bucket.HasValue)
                {
                    int index = Math.Abs(bucket.Value.Key.GetHashCode()) % _size;
                    int i = 0;
                    while (_buckets[index].HasValue)
                    {
                        i++;
